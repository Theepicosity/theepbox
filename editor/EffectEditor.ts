	...
	public render(forceRender: boolean = false): void {
		...
		if (!this._doc.changedEffect || forceRender) {
			...
			for (let effectIndex: number = 0; effectIndex < instrument.effectCount; effectIndex++) {
				...
				const panModeSelect: HTMLSelectElement = buildOptions(HTML.select(), Config.panModes);
				const clippingTypeSelect: HTMLSelectElement = buildOptions(HTML.select(), Config.clippingTypes);
				...
				const echoDelayRow: HTMLDivElement = HTML.div({ class: "selectRow", style: "display: none;" }, HTML.span({ class: "tip", onclick: () => this._openPrompt("echoDelay") }, "Delay:"), HTML.div({ style: `color: ${ColorConfig.secondaryText}; ` }, echoDelayNum), echoDelaySlider.container);
				const echoPingPongRow: HTMLDivElement = HTML.div({ class: "selectRow", style: "display: none;" }, HTML.span({ class: "tip", onclick: () => this._openPrompt("echoPingPong") }, "Ping Pong:"), echoPingPongSlider.container);
